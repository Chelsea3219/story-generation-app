// We are going to keep calling the endpoint to see if the job is finished. As soon as it is done, we'll load the
// story and redirect them to that page.
...
function StoryGenerator() {
    ...
    useEffect(() => {
        ...
        // when we leave this page OR when the component is rendered or detached from screen -->
        // function will see if polling is going on --> if yes, then it will just clear it
        return () => {
            if (pollInterval) {
    ...
    // Keep polling the job status until story is completed
    ...
    return (
        <div className="story-generator">
            ...
            {(loading || jobStatus === "processing") && (<LoadingStatus theme={theme} />)}
        </div>
    )
}
